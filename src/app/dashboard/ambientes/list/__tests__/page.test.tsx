        ...
        {data.length ? (
          data.map((row) => (
            <div key={row.id}>
              ...
              <button
                type="button"
                onClick={() => columns?.[0]?.onAssignSchedules?.(row)}
              >
                Asignar horarios {row.nombre}
              </button>
            </div>
          ))
        ) : (
...
vi.mock("@/features/environments/list/columns", () => ({
  environmentColumns: (
    ...
    onAssignSchedules: (row: EnvironmentRow) => void
  ) => [{ id: "actions", onDelete, onAssignSchedules }],
}));
...
vi.mock("@/features/environments/list/EnvironmentSchedulesDialog", () => ({
  EnvironmentSchedulesDialog: ({
    ...
    open ? (
      <div data-testid="mock-schedules-dialog">
        <p>
          dialogo de horarios para{" "}
          <span data-testid="schedules-environment-name">
            {environment?.nombre ?? "sin ambiente"}
          </span>
        </p>
...
describe("EnvironmentListPage", () => {
  ...
  it("usa catálogos completos en filtros y solo los activos en formularios de crear y editar", async () => {
    ...
    await user.click(
      await screen.findByRole("button", { name: /nuevo ambiente/i })
    );
  ...
  it("aplica los filtros ingresados y reconsulta la API con los parametros normalizados", async () => {
    ...
    apiFetchMock.mockResolvedValueOnce({
      items: [
        { id: 2, nombre: "Laboratorio 5", codigo: "LAB5" } as EnvironmentRow,
      ],
      meta: { page: 1, pages: 1, take: 8 },
    });
    ...
    // Pulsamos el boton de buscar para disparar la reconsulta.
  ...
  it("abre el dialogo de horarios con el ambiente seleccionado", async () => {
    // Preparamos cat�logos para que la pantalla tenga datos disponibles.

    queueCatalogResponses();

    // Simulamos que la API devuelve un ambiente activo en la primera carga.
    ...
    // Creamos el objeto userEvent para emular interacciones reales.

    const user = userEvent.setup();

    // Renderizamos la pagina y activamos los efectos iniciales.

    render(<EnvironmentListPage />);

    // Esperamos a que aparezca el nombre del ambiente para confirmar la carga inicial.

    await waitFor(() =>
      expect(screen.getByText("Aula 101")).toBeInTheDocument()
    );

    // Buscamos el boton que dispara la asignacion de horarios para el ambiente cargado.

    const schedulesButton = screen.getByRole("button", {
      name: "Asignar horarios Aula 101",
    });

    // Disparamos un clic sobre el boton para abrir el dialogo de horarios.

    await user.click(schedulesButton);

    // Verificamos que el dialogo mock de horarios se muestre en pantalla.

    expect(screen.getByTestId("mock-schedules-dialog")).toBeInTheDocument();

    // Comprobamos que el dialogo reciba el nombre del ambiente seleccionado.

    expect(screen.getByTestId("schedules-environment-name").textContent).toBe(
      "Aula 101"
    );

    // Cerramos el dialogo para asegurar que la vista responda al callback de cierre.

    await user.click(screen.getByRole("button", { name: "cerrar horarios" }));

    // Confirmamos que el dialogo desaparece despues de cerrar.

    await waitFor(() =>
      expect(
        screen.queryByTestId("mock-schedules-dialog")
