...
describe("BlockListPage", () => {
  ...
  it("muestra el listado inicial de bloques con los controles de busqueda y filtros", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: /bloques/i })
    ).toBeInTheDocument(); // Confirmamos que el titulo principal sea visible.
    ...
    await waitFor(() => {
      ...
      expect(
        within(dataRow!).getByText("Facultad de Tecnologia")
      ).toBeInTheDocument(); // La pertenencia a la facultad debe mostrarse.
    ...
    expect(screen.getByLabelText(/filtrar por facultad/i)).toBeInTheDocument();
    ...
    expect(screen.getByLabelText(/pisos mínimos/i)).toBeInTheDocument();

    expect(screen.getByLabelText(/pisos máximos/i)).toBeInTheDocument();
  });
  ...
  it("usa catálogos completos para filtros y solo los activos en formularios de crear/editar", async () => {
    ...
    const facultyTrigger = await screen.findByLabelText(
      /filtrar por facultad/i
    );
  ...
  it("aplica los filtros solo cuando la persona confirma con el bot?n Buscar", async () => {
    const user = userEvent.setup(); // Configuramos al usuario virtual.

    render(<BlockListPage />); // Renderizamos la pantalla.

    await screen.findByText("Bloque Central"); // Esperamos la carga inicial para contar las llamadas base.

    const initialBlockCalls = mockedApiFetch.mock.calls.filter(([path]) =>
      (path as string).startsWith("/bloques")
    ).length; // Registramos cu?ntas consultas al listado se hicieron al montar.

    const facultyTrigger = await screen.findByLabelText(
      /filtrar por facultad/i
    ); // Localizamos el trigger del select de facultades.
    ...
    }); // Buscamos la opci?n concreta por su rol accesible.
    ...
    await user.click(blockTypeOption); // Seleccionamos la opci?n administrativa.
    ...
    }); // Ubicamos la opci?n de inactivos usando su rol accesible.

    await user.click(inactiveOption); // Elegimos la opci?n de inactivos.
    ...
    await user.type(minInput, "2"); // Establecemos un l?mite inferior.

    await user.clear(maxInput); // Limpiamos el l?mite superior.

    await user.type(maxInput, "6"); // Definimos el m?ximo permitido.
    ...
    await waitFor(() => {
      ...
      expect(blockCalls.length).toBe(initialBlockCalls); // Mientras no se presione Buscar, no deber?an dispararse nuevas consultas.
    });

    await user.click(screen.getByRole("button", { name: /Buscar/i })); // Confirmamos el formulario para aplicar todos los filtros juntos.
    ...
    await waitFor(() => {
      ...
      const lastPath = blockCalls.at(-1)?.[0] as string | undefined; // Extraemos la ?ltima ruta solicitada.

      expect(blockCalls.length).toBeGreaterThan(initialBlockCalls); // Debe generarse una nueva consulta al enviar.

      expect(lastPath).toContain("facultadId=9"); // Confirmamos que la query incluya el identificador reci?n seleccionado.

      expect(lastPath).toContain("tipoBloqueId=5"); // Validamos que el filtro se traduzca en el query param esperado.

      expect(lastPath).toContain("activo=false"); // Revisamos que el estado booleano se env?e en la URL tras aplicar.

      expect(lastPath).toContain("pisosMin=2"); // La query debe incluir el rango m?nimo.

      expect(lastPath).toContain("pisosMax=6"); // Y tambi?n el rango m?ximo.

      expect(lastPath).toContain("page=1"); // Al aplicar los filtros regresamos a la primera p?gina.
    });
  });
  ...
  it("permite limpiar los filtros y vuelve a la pagina inicial", async () => {
    ...
    const facultyTrigger = await screen.findByLabelText(
      /filtrar por facultad/i
    ); // Abrimos el filtro para generar un estado distinto al inicial.
  ...
  it("abre el modal de creacion y permite disparar el guardado desde el formulario", async () => {
    ...
    expect(dialogUtils.getByTestId("mock-create-form")).toBeInTheDocument(); // Verificamos que el formulario de creacion se haya renderizado.
    ...
    await waitFor(() => {
      const blockRequests = mockedApiFetch.mock.calls.filter(
        ([path, options]) => {
          return (
            typeof path === "string" &&
  ...
  it("abre el dialogo de edicion y refresca el listado cuando el guardado simulado finaliza", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /simular guardado/i })); // Simulamos que el formulario termino correctamente y notifico al padre.
