...
# Crear tipo de ambiente - Especificaciones - HU 17

Visión y alcance:

Crear un tipo de ambiente para el inventario institucional.

Registrar un tipo de ambietne en menos de 2 minutos, con errores de validación claros.

Security by default y security by design (validación estricta, whitelisting, manejo controlado de errores).

Glosario y modelo conceptual:

Un tipo de ambiente clasifica los ambientes.

Reglas de negocio:

nombre (máx. 64), descripcion (máx. 256) son obligatorios.

descripcion_corta (max. 32) opcional.
...
{
"nombre": "Ambiente de clases",
"descripcion": "Ambiente para el dictado de clases",
"descripcion_corta": "Dictar clases"
}

Contratos de API
POST /api/tipo_ambientes -> 201 { "id": number }
400 VALIDATION_ERROR con details: [{ field, message }]

# Listar tipos de ambientes - Especificaciones - HU 18

Visión y alcance:

Listar tipos de ambientes en una tabla para consulta y gestión.
...
Un tipo de ambiente clasifica ambientes.
...
Endpoint: GET /api/tipo_ambientes
...
# Editar un tipo de ambiente - Especificaciones - HU 19

Visión y alcance:

Editar tipos de ambientes existentes para mantener información actualizada.
...
Un tipo de ambiente es el clasificador de ambientes.
...
Endpoint: PATCH /api/tipo_ambientes/{id}
...
{
"nombre": "Ambiente de clase - actualizado",
"descripcion": "En este ambiente solo se dictan clases",
"descripcion_corta": "Dictado"
"activo": Activo
}
...
descripcion: string (1..256) si se envía.

descripcion_corta: string (1...32) si se envia
...
# Eliminar un tipo de ambiente - Especificaciones - HU 20

Visión y alcance:

Eliminar tipos de ambientes existentes para mantener información actualizada.

Eliminacion en menos de 2 minutos, con mensajes de validación claros.

La Eliminacion de un tipo de ambiente debe hacerlo en cadena, por lo que eliminaria sus Ambientes

Security by default/design: validación estricta, errores tipificados y control de colisiones (conflictos por duplicados).

Glosario y modelo conceptual:

Un tipo de ambiente es un clasificador de ambientes.

Reglas de negocio:

Si se elimina un tipo de ambiente sus dependientes como sus Ambientes pasar a ser eliminados.
...
Endpoint: DELETE /api/tipo_ambientes/{id}
...
# Crear ambiente - Especificaciones - HU 21

Visión y alcance:

Crear un ambiente para el inventario institucional.

Registrar un ambiente en menos de 2 minutos, con errores de validación claros.

Security by default y security by design (validación estricta, whitelisting, manejo controlado de errores).

Glosario y modelo conceptual:

Un ambiente pertenece a un bloque y es de un tipo de ambiente.

activo es True por defecto

Al eliminar un ambiente todos los activos que tiene este ambiente pasaran a estar en null para su campo ambiente_id
...
Contratos de API
POST /api/ambientes -> 201 { "id": number }
400 VALIDATION_ERROR con details: [{ field, message }]

# Listar ambientes - Especificaciones - HU 22

Visión y alcance:

Listar ambientes en una tabla para consulta y gestión.
...
Un ambiente pertenece a un bloque y es del tipo de un tipo de ambiente.

Reglas de negocio:

El listado debe soportar paginación (page, limit).

Debe permitir filtrar informacion
...
y los filtros que se vayan a implementar
...
"items": [
{
"Aqui deberia estar un ambiente"
}
],
"meta": {
"total": 50,
...
# Editar un ambiente - Especificaciones - HU 23

Visión y alcance:

Editar ambientes existentes para mantener información actualizada.
...
Un ambiente esta en un bloque y es de un tipo de bloque.
...
Endpoint: PATCH /api/ambientes/{id}
...
# Eliminar un ambiente - Especificaciones - HU 24

Visión y alcance:

Eliminar ambientes existentes para mantener información actualizada.

Eliminacion en menos de 2 minutos, con mensajes de validación claros.

La Eliminacion de un ambiente debe poner todos los activos que pertenecen a este ambiente en null en su campo ambiente_id

Security by default/design: validación estricta, errores tipificados y control de colisiones (conflictos por duplicados).

Glosario y modelo conceptual:

Un ambiente es pertenece a un bloque y es de un tipo de ambiente.
...
Endpoint: DELETE /api/ambientes/{id}
